-- Supabase table schema for accounts

create table if not exists accounts (
  id bigint generated by default as identity primary key,
  username text not null unique,
  password_hash text not null,
  full_name text not null,
  email text not null unique,
  role text not null check (role in ('superadmin','admin','intern','supervisor')),
  school text,
  course text,
  division_id bigint,
  division_name text,
  student_id text,
  year_level text,
  status text default 'active',
  start_date date,
  end_date date,
  required_hours numeric default 486,
  rendered_hours numeric default 0,
  phone text,
  home_address text,
  emergency_name text,
  emergency_relation text,
  emergency_phone text,
  created_at timestamptz not null default now()
);

-- Division management table
create table if not exists divisions (
  id bigint generated by default as identity primary key,
  name text not null unique,
  description text,
  head_name text,
  created_at timestamptz not null default now()
);

-- Supabase table for generated office QR codes
create table if not exists qr_codes (
  id bigint generated by default as identity primary key,
  qr_code text not null unique,
  is_active boolean not null default true,
  generated_at timestamptz not null default now(),
  expires_at timestamptz not null
);

-- Supabase table for attendance logs
create table if not exists attendance_logs (
  id bigint generated by default as identity primary key,
  intern_id bigint,
  intern_name text,
  scan_type text not null check (scan_type in ('time_in','time_out')),
  scan_time timestamptz not null,
  qr_code_id bigint,
  approval_status text not null default 'pending' check (approval_status in ('pending','approved','rejected')),
  remarks text,
  created_at timestamptz not null default now()
);

-- Documents uploaded by interns
create table if not exists documents (
  id bigint generated by default as identity primary key,
  intern_id bigint not null,
  document_type text not null,
  original_name text not null,
  file_name text not null,
  file_type text not null,
  file_size bigint not null,
  file_path text not null,
  upload_date timestamptz not null default now(),
  status text not null default 'pending',
  admin_remarks text,
  created_at timestamptz not null default now()
);

-- Evaluation records for interns
create table if not exists evaluations (
  id bigint generated by default as identity primary key,
  intern_id bigint not null,
  evaluator_id bigint not null,
  evaluator_name text not null,
  evaluation_date timestamptz not null default now(),
  overall_score numeric not null,
  overall_rating text not null,
  comments text,
  work_quality numeric,
  punctuality numeric,
  teamwork numeric,
  communication numeric,
  initiative numeric,
  created_at timestamptz not null default now()
);

-- Notifications for users
create table if not exists notifications (
  id bigint generated by default as identity primary key,
  user_id bigint not null,
  title text not null,
  message text not null,
  is_read boolean not null default false,
  created_at timestamptz not null default now()
);

-- Enable Row Level Security (RLS) for notifications
alter table public.notifications enable row level security;

-- Policy: Users can read their own notifications
create policy "Users can read own notifications"
on public.notifications for select
to authenticated
using (
  (SELECT id FROM public.accounts WHERE email = auth.email()) = user_id
);

-- Policy: Users can update their own notifications (mark as read)
create policy "Users can update own notifications"
on public.notifications for update
to authenticated
using (
  (SELECT id FROM public.accounts WHERE email = auth.email()) = user_id
)
with check (
  (SELECT id FROM public.accounts WHERE email = auth.email()) = user_id
);

-- Policy: Allow admins to manage notifications for all users
create policy "Admins can manage all notifications"
on public.notifications for all
to authenticated
using (
  (SELECT role FROM public.accounts WHERE email = auth.email()) = 'admin'
)
with check (
  (SELECT role FROM public.accounts WHERE email = auth.email()) = 'admin'
);

-- Table for calendar events like holidays and announcements
create table if not exists calendar_events (
  id bigint generated by default as identity primary key,
  title text not null,
  description text,
  event_date date not null,
  event_type text not null check (event_type in ('holiday', 'announcement', 'memo', 'suspension')),
  created_by bigint references accounts(id) on delete set null,
  created_at timestamptz not null default now()
);

-- Enable Row Level Security (RLS) for the new table
alter table public.calendar_events enable row level security;

-- Policy: Allow all authenticated users to read all calendar events
create policy "Allow all authenticated users to read events"
on public.calendar_events for select
to authenticated
using (true);

-- Policy: Allow only admins to create, update, or delete events
create policy "Allow admins full access"
on public.calendar_events for all
to authenticated
using (
  (SELECT role FROM public.accounts WHERE email = auth.email()) in ('admin', 'supervisor')
)
with check (
  (SELECT role FROM public.accounts WHERE email = auth.email()) in ('admin', 'supervisor')
);

-- Intern Projects table for tracking projects, progress & group members
create table if not exists intern_projects (
  id bigint generated by default as identity primary key,
  title text not null,
  description text,
  group_name text not null,
  division_id bigint references divisions(id) on delete set null,
  status text not null default 'in_progress' check (status in ('planning', 'in_progress', 'review', 'completed', 'on_hold')),
  progress integer not null default 0 check (progress >= 0 and progress <= 100),
  leader_id bigint references accounts(id) on delete set null,
  members jsonb default '[]'::jsonb,
  github_repo text,
  demo_url text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- Enable RLS for intern_projects
alter table public.intern_projects enable row level security;

-- Role-scoped read and mutation policies are installed by
-- migration_project_access_model.sql. The API additionally verifies the
-- current database account for every project operation.

-- Project Files repository table for deliverables & project documents
create table if not exists project_files (
  id bigint generated by default as identity primary key,
  project_id bigint references intern_projects(id) on delete cascade,
  uploaded_by bigint references accounts(id) on delete set null,
  uploader_name text,
  file_category text not null default 'documentation' check (file_category in ('documentation', 'source_code', 'presentation', 'deliverable', 'other')),
  original_name text not null,
  file_name text not null,
  file_type text not null,
  file_size bigint not null,
  file_path text not null,
  upload_date timestamptz not null default now(),
  created_at timestamptz not null default now()
);

-- Enable RLS for project_files
alter table public.project_files enable row level security;

-- Project-file policies are installed by migration_project_access_model.sql.
-- The documents bucket is private and the API returns authorized signed URLs.

-- Example seed data for admin and intern users (password = "password")
insert into accounts (username, password_hash, full_name, email, role)
values
  ('admin', '$2a$10$8DA752LK9rXg3ejo67sfJOhWxQHeEkjlpDv5PxMFTfVhrG7.h1Y6O', 'Demo Admin', 'admin@pnp-itms.gov.ph', 'admin'),
  ('intern', '$2a$10$fiRc0kvZGC4cQmYQmnbJfunN7PYDRH2tNA0m0kTyo3YkCCQ03Ckm.', 'Demo Intern', 'intern@pnp-itms.gov.ph', 'intern'),
  ('superadmin', '$2a$10$StKL8H7szIFcq28NI.IwKewDMiE1qKwo6iPgZIFqmQEykMlJTvVpq', 'Super Administrator', 'superadmin@pnp-itms.gov.ph', 'superadmin')
on conflict (username) do nothing;

-- Attendance profiles table for Superadmin Attendance Control
create table if not exists attendance_profiles (
  id bigint generated by default as identity primary key,
  profile_name text not null,
  time_in time not null default '08:00:00',
  time_out time not null default '17:00:00',
  first_scan_enabled boolean not null default true,
  second_scan_enabled boolean not null default true,
  status text not null default 'active' check (status in ('active', 'inactive')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

-- Account-specific attendance profile assignment table
create table if not exists account_attendance_profiles (
  id bigint generated by default as identity primary key,
  account_id bigint not null references accounts(id) on delete cascade,
  attendance_profile_id bigint not null references attendance_profiles(id) on delete cascade,
  assigned_at timestamptz not null default now(),
  assigned_by bigint references accounts(id) on delete set null,
  unique (account_id)
);

-- Daily attendance records table
create table if not exists attendance_records (
  id bigint generated by default as identity primary key,
  account_id bigint not null references accounts(id) on delete cascade,
  attendance_date date not null,
  actual_time_in timestamptz,
  recorded_time_in timestamptz,
  actual_time_out timestamptz,
  recorded_time_out timestamptz,
  attendance_profile_id bigint references attendance_profiles(id) on delete set null,
  status text not null default 'pending' check (status in ('pending', 'approved', 'rejected')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (account_id, attendance_date)
);

-- DTR edit history table for Superadmin audit log
create table if not exists dtr_edit_history (
  id bigint generated by default as identity primary key,
  attendance_record_id bigint references attendance_records(id) on delete cascade,
  account_id bigint not null references accounts(id) on delete cascade,
  field_name text not null,
  original_value text,
  new_value text,
  modified_by bigint references accounts(id) on delete set null,
  modified_at timestamptz not null default now(),
  reason text not null
);

