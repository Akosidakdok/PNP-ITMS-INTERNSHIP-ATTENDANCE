...
export default function Divisions() {
  ...
  const handleSave = async () => {
    if (!form.name) { toast.error('Division name required'); return; }
    setSaving(true);
    try {
      if (selected) {
  ...
  const handleInternSave = async () => {
    ...
    try {
      ...
      setModal('interns');
      fetchInternsForDiv(selected.id);
      fetch(); // Refresh division counts
    } catch (err) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Modal
        ...
        title={selected ? 'Edit Division' : 'Add Division'}
        size="sm"
        footer={
          <>
      ...
      >
        <div className="space-y-4">
          <div className="form-group"><label className="form-label">Division Name *</label><input className="form-input" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></div>
          ...
          <div className="form-group"><label className="form-label">Division Head</label><input className="form-input" value={form.head_name} onChange={e => setForm(f => ({ ...f, head_name: e.target.value }))} /></div>
        </div>
      </Modal>

      {/* Delete Division Modal */}
      <Modal isOpen={modal === 'delete'} onClose={() => setModal(null)} title="Delete Division" size="sm"
        footer={
          <>
            <button className="btn btn-secondary" onClick={() => setModal(null)}>Cancel</button>
      ...
      >
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-xs text-gray-500">Manage all interns working in this division.</p>
          ...
          {internsLoading ? (
            ...
            <div className="text-center py-8 text-gray-400 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-sm">No interns currently assigned to this division.</p>
            </div>
          ) : (
